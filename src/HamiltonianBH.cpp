...
void HamiltonianBH::setExpansionOrder(size_t expansionOrder_){
  ...
  for(int i = 1; i <= N-1; ++i) {
    if (expansionOrder >= 1) {
      ...
      auto fop = -0.5*J*prefactors.at(1);
      ampo += fop,"N",i,"Adag",i+1,"A",i;
      ampo += -fop,"N",i,"Adag",i,"A",i+1;
      ampo += fop,"N",i+1,"Adag",i,"A",i+1;
      ampo += -fop,"N",i+1,"Adag",i+1,"A",i;
      ampo += fop,"Adag",i,"A",i+1,"N",i+1;
      ampo += -fop,"Adag",i,"A",i+1,"N",i;
      ampo += fop,"Adag",i+1,"A",i,"N",i;
      ampo += -fop,"Adag",i+1,"A",i,"N",i+1;
